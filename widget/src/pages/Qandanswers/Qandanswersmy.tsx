...
interface QandanswersmyProps {
  history: History;
}

interface QandanswersmyState {
  height: number;
  dataSource: any;
  isLoading: boolean;
  hasMore: boolean;
...
export class Qandanswersmy extends React.Component<QandanswersmyProps, QandanswersmyState> {
  lv: any;
  constructor(props: QandanswersmyProps) {
    super(props);
  ...
  question_answer_list = () => {
    UserService.Instance.question_answer_list().then((res: any) => {
        this.setState({
    ...
    const row = (rowData: any, sectionID: number, rowID: number) => {
      return (
        <div className="margin-t bff lh1" onClick={()=>{this.props.history.push({pathname:'/QandanswersDetails',state:{datas:JSON.stringify(rowData)}})}}>
            ...
            <div className={"padding "+(rowData.author?'':'none')}>

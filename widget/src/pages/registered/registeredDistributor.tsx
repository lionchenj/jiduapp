...
interface registeredDistributorProps {
  history: History;
  location: Location;
}

interface registeredDistributorState {
  mobile:string,
  codeCountDown:number,
  get_description:string,
  head_img:string
}
export class registeredDistributor extends React.Component<registeredDistributorProps,registeredDistributorState> {
  registerData:string
  code:string
  codeCountDownTimer:number
  constructor(props: registeredDistributorProps) {
    super(props);
  ...
  public componentDidMount() {
    UserService.Instance.get_description('2').then( (res:any)=> {
      this.setState({
  ...
  onSubmit = () => {
    ...
    UserService.Instance.apply_for(this.state.mobile,this.code,'2').then( ()=> {
      Modal.alert('提示',"申请分销商成功")
      this.props.history.push("/");
    }).catch( (err) => {
        UIUtil.showError(err)
    })
  ...
  public render() {
    return (
      <div className="address-container registered">
        ...
        >
          <div className="nav-title">申请分销商</div>
        </NavBar>
        ...
        <div className="padding bff">
          <div className="w100 padding-tb fs16 tac">分销商申请说明</div>
